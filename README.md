...
We also state that in general the sets we will construct will have a primary element, the replication of which is the purpose of the set.  There might be many other elements and subsets which are needed for replication, but the *primary element* is defined as the element the replication of which represents the *primary intent of the creator of the set*.  We thus make the human will, desire, or intent a fundamental element of our entire set theory.  

So a way to express the most general possible self-replicating set is:
